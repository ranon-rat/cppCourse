...
its more for autoincrement a constant , for somes projects this is really useful

```c++
#include <iostream>
using namespace std;
enum test:int {test,test=14};
int main(){
    for(int i :test)
        cout<<i<<endl;
    return 0;
...
in c++, the NULL macro its a little bit problematic for some functions, so the recommendation for avoid that is use nullptr

# dynamic memory operators

(this is not avaible in c)
if you are using new, you need to delete the variable later for avoid some vulnerabilities .Like this:

```c++
#include <iostream>
using namespace std;
const long int count=1400;
int main(){
    // allocate array
    long int *val;
    try {
        val=new long int [count];
    }catch(bad_alloc b){
        cout <<b.what()<<endl;
        return 1;
    }
...
its more for do the conversion between two different values with different types
...
this operator its more for get the size of the variable
, for use it we only need to do this
...
This operator its for get the variable type  in c++